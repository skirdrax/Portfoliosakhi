...
export default function Navbar() {
  ...
  // Listen to theme changes from App.jsx
  useEffect(() => {
    ...
    <>
      <style>{`
        .nav {
          ...
          width: 100%; padding: 20px 24px; box-sizing: border-box;
          background: ${bg};
        ...
        .logo::after { content:''; position:absolute; bottom:-4px; left:0; width:100%; height:1.5px; background:linear-gradient(90deg,#7c3aed,transparent); border-radius:2px; }
        .nav-links { display:flex; align-items:center; gap:32px; }
        .nav-link { font-size:13px; font-weight:500; color:${textSecondary}; text-decoration:none; padding:4px 0; position:relative; transition:color .2s; letter-spacing:.01em; }
        .nav-link::after { content:''; position:absolute; bottom:-2px; left:0; width:0; height:1.5px; background:#7c3aed; border-radius:2px; transition:width .25s cubic-bezier(.22,1,.36,1); }
        .nav-link:hover { color:${textPrimary}; }
        .nav-link:hover::after { width:100%; }
        .nav-cta { font-size:12px; font-weight:700; letter-spacing:.04em; color:#a78bfa; text-decoration:none; border:1px solid rgba(124,58,237,.3); padding:8px 18px; border-radius:8px; transition:all .2s; white-space:nowrap; }
